...
func init() {
	http.HandleFunc("/mandelbrot", frontPageHandler)
	...
	for i := range colors {
		...
		if float64(i) < k6 {
			colors[i] = color.RGBA{uint8(float64(i) * 255.0 / k6), 0, 0, 0xFF}
		} else if float64(i) < k5 {
			colors[i] = color.RGBA{0xFF, uint8(255.0 * (float64(i) - k6) / (k5 - k6)), 0, 0xFF}
		} else if float64(i) < k4 {
			colors[i] = color.RGBA{uint8(255.0 - (255.0 * (float64(i) - k5) / (k4 - k5))), 0xFF, 0, 0xFF}
		} else if float64(i) < k3 {
			colors[i] = color.RGBA{0, 0xFF, uint8(255.0 * (float64(i) - k4) / (k3 - k4)), 0xFF}
		} else if float64(i) < k2 {
			colors[i] = color.RGBA{0, uint8(255.0 - (255.0 * (float64(i) - k3) / (k2 - k3))), 0xFF, 0xFF}
		} else if float64(i) < k1 {
			colors[i] = color.RGBA{uint8(255.0 * (float64(i) - k2) / (k1 - k2)), 0, 0xFF, 0xFF}
		} else {
			colors[i] = color.RGBA{0xFF, uint8(255.0 * (float64(i) - k1) / (k0 - k1)), 0xFF, 0xFF}
		}
	}
}
